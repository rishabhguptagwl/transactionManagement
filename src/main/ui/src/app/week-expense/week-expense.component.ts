...
@Component({
  selector: 'app-week-expense',
  templateUrl: './week-expense.component.html',
  styleUrls: ['./week-expense.component.css']
})
export class WeekExpenseComponent implements OnInit {
  formDate: string;
  toDate: string;
  ...
  ngOnInit() {
    ...
    this.arr = [];
    this.total = [];
    this._weekExpense.getTransactionWeekly(this.pipe.transform(Date.now(), 'yyyy-MM-dd')).subscribe(data => {
      this.populatepage(data);
    });
    this._weekExpense.getTransactionByClass(this.pipe.transform(Date.now(), 'yyyy-MM-dd')).subscribe(data => {
      this.TransactionClasses(data);
    });


  }
  TransactionClasses(data: any) {
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i].Transactions.length; j++) {
        if (data[i].Transactions[j].value === "Essentials") {
          essential += (data[i].Transactions[j].amount);
        }
        else {
          lifestyle += (data[i].Transactions[j].amount);
        }
      }
    }
    var t = [];
    t.push({ "region": "essentials", "count": essential }, { "region": "Life Style", "count": lifestyle });
    // alert("going to call pie chart");
    this.pie({ "value": t }, "piechart");
  }
  populatepage(data) {
    console.log(data);
    this.arrDailyTransaction = data;
    this.formDate = (this.arrDailyTransaction[0].date) + "";
    this.toDate = (this.arrDailyTransaction[this.arrDailyTransaction.length - 1].date) + "";
    var chartData = "";
    for (let i = 0; i < data.length; i++) {
  ...
  previousdate() {
    ...
    this.formDate = (this.pipe.transform(date1, "dd-MM-yyyy"));
    this.arr = [];
    this.total = [];
    this._weekExpense.getTransactionWeekly(this.pipe.transform(date1, "yyyy-MM-dd")).subscribe(data => {
      this.populatepage(data);
    });
    this._weekExpense.getTransactionByClass(this.pipe.transform(date1, 'yyyy-MM-dd')).subscribe(data => {
      this.TransactionClasses(data);
    });
  }

  nextDate() {
    var tmp = this.toDate.split("-");
    var date1 = new Date(this.pipe.transform(tmp[1] + "-" + tmp[0] + "-" + tmp[2]));
    date1.setDate(date1.getDate() + 1);
    this.formDate = (this.pipe.transform(date1, "dd-MM-yyyy"));
    this.arr = [];
  ...
  setValues() {
    var data = [];
    var expense = [];
    for (let t = 0; t < this.transaction.length; t++) {
      data.push({ "date": this.transaction[t].date, "trans": JSON.parse(this.transaction[t].transaction + "") });
      var tmp = JSON.parse(this.transaction[t].transaction + "");
      console.log(tmp);
    }
  }
